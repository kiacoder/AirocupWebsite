...
const App = {
  ...
  InitializeDeleteConfirmation() {
    ...
    document.body.addEventListener("click", (event) => {
      ...
      // Dynamically change modal content based on what is being deleted
      if (memberName) {
  ...
  InitializeDocumentUpload() {
    ...
    FileInput.addEventListener("change", () => {
      ...
      const VideoMaxSize = 300 * 1024 * 1024; // 300 MB
      const ImageMaxSize = 50 * 1024 * 1024; // 50 MB
      const DocumentMaxSize = 100 * 1024 * 1024; // 100 MB
...
const Constants = {
  Selectors: {
    ...
    RelativeTime: "[data-timestamp].RelativeTime, [data-timestamp].relative-time",
  ...
  Messages: {
    ...
    TeamNameInvalidChars: "نام تیم فقط می‌تواند شامل حروف، اعداد، فاصله و خط زیر باشد.",
    ...
    SocketWarning: "اتصال لحظه‌ای برقرار نیست — پیام فقط محلی نمایش داده می‌شود.",
...
const UI = {
  InitializeMobileMenu() {
    const MenuButton = document.querySelector(Constants.Selectors.MobileMenuBtn);
    ...
    const ToggleMenu = (ShouldShow) => {
      ...
      document.body.classList.toggle(Constants.Classes.BodyNoScroll, ShouldShow);
    };
  ...
  InitializeGalleryModal() {
    ...
    document.querySelectorAll(Constants.Selectors.GalleryItemImg).forEach((Image) => {
    ...
    Modal.querySelector(".CloseBtn")?.addEventListener(Constants.Events.Click, () => {
      ModalHelpers.CloseModal(Modal);
    });
    ...
    document.addEventListener(Constants.Events.Keydown, (Event) => {
      if (Event.key === "Escape" && Modal.classList.contains(Constants.Classes.IsVisible)) {
        ModalHelpers.CloseModal(Modal);
      }
    });
  },
